...
import { isNotificationInput } from '../notification-capability.js';
...
export class NotificationService {
  ...
  async list(): Promise<readonly NotificationView[]> {
    ...
    const acknowledged = new Set(
      events
        .filter((event) => event.key === 'notificationAcknowledged')
        .flatMap((event) => (event.causation.intentId ? [event.causation.intentId] : [])),
    );
