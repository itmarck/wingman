...
import type { DetectorContext, DetectorFinding, SuggestionDetector } from '../domain/detector.js';
...
function missingNextAction(): SuggestionDetector {
  return detector(
    'missingNextAction',
...
function blockerDuration(windowMs: number): SuggestionDetector {
  return detector(
    'blockerDuration',
...
function deadlineRisk(leadMs: number): SuggestionDetector {
  return detector(
    'deadlineRisk',
...
function inactivity(windowMs: number): SuggestionDetector {
  return detector(
    'inactivity',
...
function conflict(): SuggestionDetector {
  return detector(
    'conflict',
...
function relevantChange(): SuggestionDetector {
  return detector(
    'relevantChange',
...
function detector(
  key: string,
  description: string,
  dependencies: SuggestionDetector['dependencies'],
  detect: SuggestionDetector['detect'],
): SuggestionDetector {
